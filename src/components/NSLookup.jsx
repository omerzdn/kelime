        ...
        try {
            ...
            if (useRdapFallback) {
                ...
                if (rdapResponse.ok) {
                    ...
                    // Build WHOIS-style raw text
                    let rawText = '';
                    
                    // Domain info
                    rawText += `Domain Name: ${data.ldhName || domainToLookup.toUpperCase()}\n`;
                    if (data.handle) rawText += `Registry Domain ID: ${data.handle}\n`;
                    
                    // Find registrar info
                    let registrar = '', registrarIANA = '', registrarEmail = '', registrarPhone = '';
                    let registrant = { org: '', state: '', country: '' };
                    ...
                    if (data.entities) {
                        for (const entity of data.entities) {
                            ...
                            if (roles.includes('registrar')) {
                                if (entity.vcardArray) {
                                    for (const prop of entity.vcardArray[1] || []) {
                                        if (prop[0] === 'fn') registrar = prop[3];
                                        if (prop[0] === 'email') registrarEmail = prop[3];
                                        if (prop[0] === 'tel') registrarPhone = prop[3];
                                    }
                                }
                                if (entity.publicIds) {
                                    for (const id of entity.publicIds) {
                                        if (id.type === 'IANA Registrar ID') registrarIANA = id.identifier;
                                    }
                                }
                            }
                            if (roles.includes('registrant')) {
                                if (entity.vcardArray) {
                                    for (const prop of entity.vcardArray[1] || []) {
                                        if (prop[0] === 'org') registrant.org = prop[3];
                                        if (prop[0] === 'adr' && Array.isArray(prop[3])) {
                                            registrant.state = prop[3][4] || '';
                                            registrant.country = prop[3][6] || '';
                                        }
                                    }
                                }
                            }
                    ...
                    rawText += `Registrar WHOIS Server: \n`;
                    rawText += `Registrar URL: \n`;
                    
                    // Dates
                    let creationDate = '', expiryDate = '', updatedDate = '';
                    if (data.events) {
                    ...
                    rawText += `Updated Date: ${updatedDate}\n`;
                    rawText += `Creation Date: ${creationDate}\n`;
                    rawText += `Registry Expiry Date: ${expiryDate}\n`;
                    rawText += `Registrar: ${registrar}\n`;
                    rawText += `Registrar IANA ID: ${registrarIANA}\n`;
                    rawText += `Registrar Abuse Contact Email: ${registrarEmail}\n`;
                    rawText += `Registrar Abuse Contact Phone: ${registrarPhone}\n`;
                    
                    // Status
                    if (data.status) {
                        for (const s of data.status) {
                            rawText += `Domain Status: ${s}\n`;
                        }
                    }
                    
                    // Registrant
                    rawText += `Registry Registrant ID: \n`;
                    rawText += `Registrant Name: \n`;
                    rawText += `Registrant Organization: ${registrant.org}\n`;
                    rawText += `Registrant Street: \n`;
                    rawText += `Registrant City: \n`;
                    rawText += `Registrant State/Province: ${registrant.state}\n`;
                    rawText += `Registrant Postal Code: \n`;
                    rawText += `Registrant Country: ${registrant.country}\n`;
                    rawText += `Registrant Phone: \n`;
                    rawText += `Registrant Phone Ext: \n`;
                    rawText += `Registrant Fax: \n`;
                    rawText += `Registrant Fax Ext: \n`;
                    rawText += `Registrant Email: \n`;
                    
                    // Admin (usually redacted)
                    rawText += `Registry Admin ID: \n`;
                    rawText += `Admin Name: \n`;
                    rawText += `Admin Organization: \n`;
                    rawText += `Admin Street: \n`;
                    rawText += `Admin City: \n`;
                    rawText += `Admin State/Province: \n`;
                    rawText += `Admin Postal Code: \n`;
                    rawText += `Admin Country: \n`;
                    rawText += `Admin Phone: \n`;
                    rawText += `Admin Phone Ext: \n`;
                    rawText += `Admin Fax: \n`;
                    rawText += `Admin Fax Ext: \n`;
                    rawText += `Admin Email: \n`;
                    
                    // Tech (usually redacted)
                    rawText += `Registry Tech ID: \n`;
                    rawText += `Tech Name: \n`;
                    rawText += `Tech Organization: \n`;
                    rawText += `Tech Street: \n`;
                    rawText += `Tech City: \n`;
                    rawText += `Tech State/Province: \n`;
                    rawText += `Tech Postal Code: \n`;
                    rawText += `Tech Country: \n`;
                    rawText += `Tech Phone: \n`;
                    rawText += `Tech Phone Ext: \n`;
                    rawText += `Tech Fax: \n`;
                    rawText += `Tech Fax Ext: \n`;
                    rawText += `Tech Email: \n`;
                    
                    // Name servers
                    if (data.nameservers) {
                        for (const ns of data.nameservers) {
                            rawText += `Name Server: ${ns.ldhName || ns}\n`;
                        }
                    }
                    
                    // DNSSEC
                    rawText += `DNSSEC: ${data.secureDNS?.delegationSigned ? 'signed' : 'unsigned'}\n`;
                    rawText += `Source: RDAP\n`;
                    
                    // Set parsed data
                    parsed.domainName = data.ldhName || domainToLookup.toUpperCase();
                    parsed.registrar = registrar;
                    parsed.creationDate = creationDate;
                    parsed.expiryDate = expiryDate;
                    parsed.registrantOrganization = registrant.org;
    ...
    return (
        <div className="ns-container">
            ...
            <main className="ns-main">
                ...
                {!hasSearched && (
                    <div className="hero">
                        <Link to="/" className="hero-home">← Ana Sayfa</Link>
                        <h1>NsLookup.io</h1>
                        <p>DNS Lookup Tool</p>
